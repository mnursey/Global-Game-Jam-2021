...
export var seconds_btw_songs = 0.5
export var fade_range = 3.0
export var fade_interval = 15.0
export var fade_hard_interval = 100.0

var combat_music = load("res://Sound/finalspacefull.wav")
var idle_music = load("res://Sound/finalspacereduction.wav")

signal fadeout_finished

onready var bgm_timer = $bmg_timer
onready var bgm_streamer = $bmg_streamer
...
func _process(delta):
	if Input.is_action_just_pressed("dash"):
		...
		print("playing combat")
	if Input.is_action_just_pressed("jump"):
		play_song(idle_music)
		print("playing idle")
